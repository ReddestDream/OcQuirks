...
package() {
  ...
  local ver=$(cat Include/Protocol/OcQuirks.h | grep OCQUIRKS_PROTOCOL_REVISION | cut -f4 -d' ')
...
if [ ! -d "Binaries" ]; then
  ...
  cd Binaries || exit 1
  ln -s ../UDK/Build/OcQuirks/RELEASE_XCODE5/X64 RELEASE || exit 1
  ln -s ../UDK/Build/OcQuirks/DEBUG_XCODE5/X64 DEBUG || exit 1
  ln -s ../UDK/Build/OcQuirks/NOOPT_XCODE5/X64 NOOPT || exit 1
  cd .. || exit 1
fi
...
if [ "$SKIP_BUILD" != "1" ]; then
  if [ "$MODE" = "" ] || [ "$MODE" = "DEBUG" ]; then
    build -a X64 -b DEBUG -t XCODE5 -p OcQuirks/OcQuirks.dsc || exit 1
  fi

  if [ "$MODE" = "" ] || [ "$MODE" = "NOOPT" ]; then
    build -a X64 -b NOOPT -t XCODE5 -p OcQuirks/OcQuirks.dsc || exit 1
  fi

  if [ "$MODE" = "" ] || [ "$MODE" = "RELEASE" ]; then
    build -a X64 -b RELEASE -t XCODE5 -p OcQuirks/OcQuirks.dsc || exit 1
  fi
fi
